...
selenium_url = 'https://www.hktvmall.com/hktv/zh/%E8%B6%85%E7%B4%9A%E5%B7%BF%E5%A0%B4/%E8%B6%85%E7%B4%9A%E5%B8%82%E5%A0%B4/%E9%A3%B2%E5%93%81-%E5%8D%B3%E6%B2%96%E9%A3%B2%E5%93%81/main/search?q=%3Arelevance%3Astreet%3Amain%3Acategory%3AAA11220000000'
driver = webdriver.Chrome()
driver.get(selenium_url)
driver.maximize_window()
time.sleep(3)
total_page = driver.find_element(By.CLASS_NAME , "total")
total_page = total_page.text
total_page = int(re.findall(r'\d+', total_page)[0])
driver.quit()

# start scraping data via requests
url = 'https://cate-search.hktvmall.com/query/products'
myobj = [] # collect the payload of the pages
for index in range(total_page): # this is the number of pages you want to scrape
    temp_dict = {
      'query': ':relevance:street:main:category:AA11220000000:',
      'currentPage': str(index), # this is the page number
      'pageSize': '60',
...
for index in range(total_page):
    try:
        x = requests.post(url, data = myobj[index]) # sending requests
...
for index, df in enumerate(product_list):
    try:
        ...
        print(f"Some columns not found in {index}th DataFrame {e}")
        continue
...
name = current_date + '_Hktvmall.csv'
